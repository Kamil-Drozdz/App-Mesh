...
import { generateData } from '@/data/cards/dataCompany';
import { generateTransactionData } from '@/data/cards/dataTransaction';
	...
	return (
		<CardContainer className='basis-1/3'>
			<div className='space-y-3 flex justify-between items-center w-full'>
				<div className='text-white'>{t('Transaction')}</div>
				<BiDotsVertical size={IconSize.basic} />
			</div>
			<ul className='flex flex-col text-center  overflow-y-auto h-[360px]'>
				{data.map((item, index) => (
					<li key={index}>
