...
const CheckoutAdressOption = ({ formData, setActiveStep, errors }) => {
	return (
		<div className='relative dark:bg-mediumBlue bg-white p-4 rounded-lg space-y-3 h-fit'>
			...
			<Button onClick={() => setActiveStep('Payment')} disabled={!Object.keys(errors).length} className='!bg-violet-500 disabled:opacity-20 hover:!bg-violet-400 !text-white space-x-2 w-full'>
