...
const CardCompanyItem = ({ item }) => {
	return (
		<dl className='flex items-center justify-center w-full px-4 border-b-[1px] border-gray-600 my-1 py-1'>
			<div className='basis-1/5 flex space-x-4 text-left'>
				<dt>
					<img className='w-8 h-8 rounded-lg' src={item.photo} alt={`${item.category} photo`} />
				</dt>
				<div className='flex  flex-col '>
					<dd>{item.name}</dd>
					<dd className='text-[12px] text-gray-500'>{item.email}</dd>
				</div>
			</div>
			<dd className='basis-1/5 flex items-center justify-center'>
				<div className={` bg-[#28c76f] text-[#28c76f] w-12 h-12 rounded-full bg-opacity-25 flex justify-center items-center `}> {item.icon}</div>
