			...
			<div className='max-h-40 overflow-y-auto'>
				{cart.map(item => (
					<div className='flex flex-col relative group' key={item.id}>
						...
						<Separator />
						<AiOutlineClose className='absolute hidden group-hover:block  hover:text-violet-500 right-2 top-2 cursor-pointer' onClick={() => removeFromCart(item.id)} size={IconSize.basic} />
					</div>
				))}
			</div>
			<Separator />
			<div className='w-full flex justify-between p-4'>
				<p className='font-semibold '>Total :</p> <p className='dark:text-white text-black'>{totalAmount.toFixed(2)}$</p>
			</div>
