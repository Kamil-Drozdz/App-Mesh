...
export default function StatusBadge({ status, className, ...props }: StatusBadgeProps) {
	return (
		<div className={clsx('flex items-center justify-center border-darkBlue rounded-full border-[2px] h-[11px] w-[11px]', status === UserStatuses.Online && 'bg-green-600', status === UserStatuses.Offline && 'dark:bg-gray-500', status === UserStatuses.DND && 'dark:bg-red-600', status === UserStatuses.Idle && 'dark:bg-yellow-600', className)} {...props}>
