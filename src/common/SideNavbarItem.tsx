...
const SideNavbarItem = ({ icon, handleActiveDropdown, activeDropdown, name, href, dropdown, menuCollapsed }) => {
	...
	return (
		<>
			{dropdown?.length ? (
				<div onClick={() => handleActiveDropdown(name)} className={`${open && 'bg-darkBlue bg-opacity-70'} ${isActiveDropdownItem && 'bg-darkBlue'} ${menuCollapsed ? 'justify-center' : 'justify-between'} flex items-center my-1 px-4 py-2.5 hover:bg-darkBlue focus:bg-darkBlue mx-3 rounded-lg transition-colors duration-150 ease-in-out cursor-pointer`}>
