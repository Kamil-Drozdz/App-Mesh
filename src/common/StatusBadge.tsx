...
export default function StatusBadge({ status, className, ...props }: StatusBadgeProps) {
  return (
    <div
      ...
      {...props}
    >
      {status === UserStatuses.Offline && <div className='h-1.5 w-1.5 rounded-full bg-stone-800' />}
      {status === UserStatuses.DND && <div className='h-0.5 w-1.5 rounded-sm bg-stone-800' />}
      {status === UserStatuses.Idle && (
        <div className='absolute -left-0.5 -top-0.5 h-2 w-2 rounded-full bg-stone-800' />
      )}
    </div>
  );
}
