					...
					<TableRow key={index}>
						...
						<TableCell className='font-medium  '>
							<div className='flex items-center space-x-2 '>
								{user?.role === 'Admin' ? <SiHashicorp size={IconSize.basic} className='text-red-400' /> : <BiUser size={IconSize.basic} className='text-blue-400' />}
								<p>{user?.role}</p>
							</div>
						</TableCell>
						<TableCell className='font-medium'>{user?.plan}</TableCell>
						<TableCell className='font-medium'>
							<p className='text-red-600 px-2 bg-red-600 bg-opacity-30 w-fit text-center rounded-lg'>{user?.emailVerified}</p>
						</TableCell>
						<Popover>
							...
							<PopoverContent className='w-auto  min-w-[120px] p-0 z-[52] bg-lightBlue flex flex-col justify-center items-center shadow-md' align='center'>
								{actions.map(action => (
