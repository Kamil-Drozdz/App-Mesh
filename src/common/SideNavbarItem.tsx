...
const SideNavbarItem = ({ icon, handleActiveDropdown, activeDropdown, name, href, dropdown, menuCollapsed }) => {
	...
	return (
		<>
			{dropdown?.length ? (
				<div onClick={() => handleActiveDropdown(name)} className={`${open && 'dark:bg-darkBlue bg-lightWhite bg-opacity-70'} ${isActiveDropdownItem && 'dark:bg-darkBlue bg-lightWhite'} ${menuCollapsed ? 'justify-center' : 'justify-between'} flex items-center my-1 px-4 py-2.5 hover:dark:bg-darkBlue focus:dark:bg-darkBlue mx-3 rounded-lg transition-colors duration-150 ease-in-out cursor-pointer`}>
				...
				<Link to={href} className={`${isActiveItem && 'dark:bg-darkBlue'} flex items-center ${menuCollapsed ? 'justify-center' : 'justify-between'} my-1 px-4 py-2.5 hover:dark:bg-darkBlue focus:dark:bg-darkBlue mx-3 rounded-lg transition-colors duration-150 ease-in-out`}>
