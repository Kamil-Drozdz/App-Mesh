					...
					{icons.map((item, index) => (
						<Tooltip key={index}>
							<TooltipTrigger asChild>
								<Button variant='empty' className={`${index !== icons.length - 1 ? 'border-r' : 'border-none'} border-gray-600 px-3 h-fit !py-0 !rounded-none`} onClick={() => handleProviderLogin(item.tooltip.toLowerCase())}>
									{item.icon}
								</Button>
							</TooltipTrigger>
